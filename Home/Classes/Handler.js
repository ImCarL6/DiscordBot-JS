...
class Handler {
      ...
      res.forEach(function (file) {
        promise = promise.then(function () {
          ...
          if (cmd?.aliases?.length > 0)
            cmd.aliases.forEach((x) => client.slashCommands.set(x, cmd));
          ...
          if (Array.isArray(cmd?.guild) && cmd?.guild?.length > 0) {
            cmd.guild.forEach((g) => {
              ...
              if (cmd?.aliases) {
                cmd.aliases.forEach((alias) => {
                  if (guild.commands.cache.find((x) => x.name == alias))
                    guild.commands.edit(
                      guild.commands.cache.find((x) => x.name == alias).id,
                      {
                        name: alias,
                ...
                if (guild.commands.cache.find((x) => x.name == cmd.name))
                  guild.commands.edit(
                    guild.commands.cache.find((x) => x.name == cmd.name).id,
                    {
                      name: cmd.name,
                      description: cmd?.description ?? "Slash command :D",
                      options: cmd?.options ?? [],
            ...
            if (creator.commands.cache.find((x) => x.name == cmd.name))
              creator.commands.edit(
                creator.commands.cache.find((x) => x.name == cmd.name).id,
                {
                  name: cmd.name,
                  description: cmd?.description ?? "Slash command :D",
                  options: cmd?.options ?? [],
  ...
  static loadErrorManager() {
    ...
    process.on("unhandledRejection", (error) => {
      ...
      console.log(
        chalk.bold.red("[Error Message] ") + chalk.blue(err[0].trim())
      );
      console.log(
        chalk.bold.red("[Error Location] ") + chalk.blue(err[1].trim())
      );
    }); // Process End
